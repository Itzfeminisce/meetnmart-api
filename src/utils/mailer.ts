...
const GOOGLE_REFRESH_TOKEN = getEnvVar("GOOGLE_REFRESH_TOKEN")
const SUPPORT_EMAIL = getEnvVar("SUPPORT_EMAIL")

if (!EMAIL_FROM || !GOOGLE_CLIENT_ID || !GOOGLE_CLIENT_SECRET || !GOOGLE_REFRESH_TOKEN) {
  throw new Error('[Mailer] Missing required email configuration');
}
...
class Mailer {
  ...
  private readonly tokenCacheKey = {
    access: "smtp:access_token",
    refresh: "smtp:refresh_token",
    expiry: "smtp:token_expiry"
  };
  ...
  private async setupTransporter(): Promise<void> {
    try {
      logger.info('setupTransporter', 'Starting transporter setup');

      await this.manageOAuthTokens();
      ...
      logger.info('setupTransporter', 'Transporter setup completed');
    } catch (error) {
  ...
  private async manageOAuthTokens(): Promise<void> {
    const [cachedRefreshToken, cachedAccessToken, cachedExpiry] = await Promise.all([
      cacheService.get<string>(this.tokenCacheKey.refresh),
      cacheService.get<string>(this.tokenCacheKey.access),
      cacheService.get<number>(this.tokenCacheKey.expiry)
    ]);

    const shouldRefresh = !cachedAccessToken ||
      (cachedExpiry && Date.now() >= cachedExpiry - 300000);

    if (shouldRefresh) {
      await this.refreshAndCacheTokens(cachedRefreshToken || GOOGLE_REFRESH_TOKEN);
    } else {
      logger.debug('manageOAuthTokens', 'Using valid cached tokens');
      ...
      this.tokenExpiresAt = cachedExpiry || 0;
    }
  }

  private async refreshAndCacheTokens(refreshToken: string): Promise<void> {
    try {
      logger.info('refreshAndCacheTokens', 'Refreshing OAuth tokens');
      ...
      // Update the local expiry
      this.tokenExpiresAt = credentials.expiry_date || (Date.now() + 3600 * 1000);

      const cachePromises = [
        cacheService.set(this.tokenCacheKey.access, credentials.access_token),
        cacheService.set(this.tokenCacheKey.expiry, this.tokenExpiresAt)
      ];

      if (credentials.refresh_token) {
        cachePromises.push(
          cacheService.set(this.tokenCacheKey.refresh, credentials.refresh_token)
        );
      }

      await Promise.all(cachePromises);
      logger.debug('refreshAndCacheTokens', 'Tokens refreshed and cached, expires at',
        new Date(this.tokenExpiresAt).toISOString());
    } catch (error) {
      logger.error('refreshAndCacheTokens', 'Token refresh failed', error);
  ...
  private createConfiguredTransporter(): Mail {
    return nodemailer.createTransport({
      ...
      auth: {
        ...
        expires: this.oAuth2Client.credentials.expiry_date
      },
      pool: true,
      maxConnections: 3,
      maxMessages: 50,
  ...
  private setupTransporterEventListeners(): void {
    this.transporter!
      ...
      .on('token', async (token) => {
        logger.debug('tokenRefresh', 'Received new token from transport');
        const expiry = Date.now() + (token.expires || 3600) * 1000;
        await this.cacheToken(token.accessToken, expiry);
      })
      .on('idle', () => logger.debug('transportState', 'Transport idle'));
  }

  private async cacheToken(accessToken: string, expiry: number): Promise<void> {
    await Promise.all([
      cacheService.set(this.tokenCacheKey.access, accessToken),
      cacheService.set(this.tokenCacheKey.expiry, expiry)
    ]);

    // Update the local expiry tracker
    ...
    logger.debug('cacheToken', `Token cached, expires at ${new Date(expiry).toISOString()}`);
  }
  ...
  private handleInitializationError(error: unknown): void {
    ...
    cacheService.del(this.tokenCacheKey.access).catch(() => { });
  }
  ...
  private async ensureHealthyTransporter(): Promise<Mail> {
    ...
    if (this.tokenExpiresAt && Date.now() > (this.tokenExpiresAt - 300000)) {
      logger.info('ensureHealthyTransporter', 'Token expiring soon, reinitializing');
      this.transporter = undefined;
    }
  ...
  private async getPlatformName(): Promise<string | null> {
    return cacheService.get<string>("config:platformName")
      .catch(() => 'Our Platform');
  }
  ...
  private handleEmailError(error: unknown, recipient: string): void {
    ...
    if (errorMessage.includes('auth') || errorMessage.includes('OAuth') ||
      errorMessage.includes('token') || errorMessage.includes('timeout')) {
  ...
  public async sendTemplateEmail(config: EmailConfig): Promise<SentMessageInfo> {
    ...
    try {
      ...
      const [platformName, appUrl] = await this.getPlatformConfig();
  ...
  private async getPlatformConfig(): Promise<[string | null, string | null]> {
    return Promise.all([
      cacheService.get<string>("config:platformName").catch(() => 'Our Platform'),
      cacheService.get<string>("config:app_url").catch(() => 'localhost:4000')
    ]);
  }
  ...
  private createDefaultTemplateData(platformName: string, appUrl: string): TemplateData {
    return {
      ...
      app_logo_url: `${appUrl}/assets/logo.png`,
      login_url: `${appUrl}/en/login?utm_source=email`,
  ...
  private async sendEmailWithRetry(options: Mail.Options): Promise<SentMessageInfo> {
    return this.retry(
